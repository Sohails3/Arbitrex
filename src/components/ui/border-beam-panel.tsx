...
/* Border Beam Panel — from Motiq (https://motiq.dev/components/border-beam-panel).
   ...
   Two changes from upstream, both because this repo already provides the thing
   being inlined:
   - `cn` is imported from @/lib/utils rather than redefined (identical impl).
   - The MOTIQ_TOKENS <style> injection is dropped; the --motiq-* tokens live in
     globals.css mapped to the Arbitrex palette, as upstream's own comment
     suggests. Injecting them per instance would also have re-emitted the same
     token block once per card. */
...
export function BorderBeamPanel({
  ...
  // Upstream assigns this during render, which react-hooks/refs rejects (and
  // React discourages). Every reader — surge, settle, the static-mode effect —
  // runs after commit, so syncing in an effect is equivalent and legal.
